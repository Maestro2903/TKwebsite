...
const CrowdCanvas = ({ src, rows = 15, cols = 7 }: CrowdCanvasProps) => {
  ...
  // Memoize config to prevent dependency array warnings
  const config = useMemo(() => ({ src, rows, cols }), [src, rows, cols]);
  ...
  useEffect(() => {
    ...
    // Scale factor for peeps (0.5 = 50% smaller)
    ...
    const normalWalk = ({ peep, props }: { peep: any; props: any }) => {
      ...
      const xDuration = 12; // Slightly longer for smoother movement
      const yDuration = 0.3; // Slower bobbing for more natural look

      const tl = gsap.timeline();
      tl.timeScale(randomRange(0.6, 1.4)); // More consistent speeds
      tl.to(
        peep,
        {
          duration: xDuration,
      ...
      tl.to(
        peep,
        {
          ...
          yoyo: true,
          y: startY - 6, // Less bobbing for smoother look
          ease: "sine.inOut", // Smooth easing for bobbing
        },
        0,
      );
    ...
    const createPeep = ({
      ...
      const peep: Peep = {
        ...
        setRect: (rect: number[]) => {
          peep.rect = rect;
          // Scale down the peeps by 50%
          peep.width = rect[2] * SCALE_FACTOR;
          peep.height = rect[3] * SCALE_FACTOR;
