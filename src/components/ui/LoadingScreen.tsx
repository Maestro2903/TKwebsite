...
// Exact CSS keyframes replicated from the original Takshashila HTML
const ANIMATION_CSS = `
  .main_logo_1 {
    z-index: 1000;
...
export default function LoadingRemastered({
  ...
  return <CinematicSpinDrawFillLoader onComplete={onFinished} />;
}

function CinematicSpinDrawFillLoader({
  onComplete,
}: {
  onComplete?: () => void;
}) {
  ...
  useEffect(() => {
    // Let the animation loop once (~4s), then start exit
  ...
  return (
    <>
      {/* Inject CSS keyframes — exact replica from the original site */}
      <style dangerouslySetInnerHTML={{ __html: ANIMATION_CSS }} />

      <AnimatePresence>
        {isVisible && (
          <motion.div
            className="fixed inset-0 z-[9999] flex flex-col flex-nowrap items-center justify-center overflow-hidden"
            initial={{ opacity: 1 }}
          ...
          >
            {/* SVG — exact replica of the original curtain loader */}
            <svg
              width="1000"
              height="1000"
              viewBox="0 0 1000 1000"
              fill="none"
              className="main_logo_1"
              style={{
                scale: "0.3",
                fill: "transparent",
                opacity: 0,
